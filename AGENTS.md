...
所有角色定义位于 `.Codex/agents/` 目录下,以下是预设角色:
...
- **vigor-dev-team**: Vigor 数据采集子系统开发团队
  - 详情见 `.Codex/agents/team-config.md`
  - **团队配置**: `.Codex/teams/vigor-dev-team/config.json`
  - **任务状态**: `.Codex/tasks/vigor-dev-team/*.json`
  - **消息收件箱**: `.Codex/teams/vigor-dev-team/inboxes/`

### 存储位置

所有 Codex 配置与运行时数据均存储在**当前项目目录** `.Codex/` 下。Codex 默认读写 `~/.Codex/teams/<team-name>` 和 `~/.Codex/tasks/<team-name>`,本项目通过**符号链接**把这两个系统路径指向项目目录:

```
~/.Codex/teams/vigor-dev-team  →  .Codex/teams/vigor-dev-team
~/.Codex/tasks/vigor-dev-team  →  .Codex/tasks/vigor-dev-team
```
...
```bash
cd <project-dir>
mkdir -p ~/.Codex/teams ~/.Codex/tasks
ln -sfn "$PWD/.Codex/teams/vigor-dev-team" ~/.Codex/teams/vigor-dev-team
ln -sfn "$PWD/.Codex/tasks/vigor-dev-team" ~/.Codex/tasks/vigor-dev-team
```

详见 `.Codex/teams/README.md`
...
│   ├── teams/                 # 团队配置(权威位置,~/.Codex 通过 symlink 指向此)
│   │   ├── README.md          # 说明文档
│   │   └── vigor-dev-team/
│   │       ├── config.json
│   │       └── inboxes/       # 成员消息收件箱
│   └── tasks/                 # 任务状态(权威位置,~/.Codex 通过 symlink 指向此)
│       └── vigor-dev-team/
│           └── *.json
├── docs/
│   └── superpowers/
